...
int cache_init(struct v3_vm_info * vm, v3_cfg_tree_t * cfg) {
...
int cache_deinit(struct v3_vm_info * vm) {
    return -1;
}


int cache_local_init(struct guest_info * core) {
...
int cache_activate_shdw_pt(struct guest_info * core) {
...
int cache_invalidate_shdw_pt(struct guest_info * core) {
    // wipe everything...
...
int cache_handle_pf(struct guest_info * core, addr_t fault_addr, pf_error_t error_code) {
...
int cache_handle_invlpg(struct guest_info * core, addr_t vaddr) {
...
struct v3_shdw_pg_impl cache_impl = {
    .name = "SHADOW_CACHE",
    .init = cache_init, 
    .deinit = cache_deinit, 
