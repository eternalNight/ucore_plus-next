...
struct shadow_page_data * create_new_shadow_pt(struct guest_info * core);
...
int vtlb_init(struct v3_vm_info * vm, v3_cfg_tree_t * cfg) {

    V3_Print("VTLB initialization\n");
    return 0;
}

int vtlb_deinit(struct v3_vm_info * vm) {
    return 0;
}

int vtlb_local_init(struct guest_info * core) {
    struct v3_shdw_pg_state * state = &(core->shdw_pg_state);
...
int vtlb_local_deinit(struct guest_info * core) {
    struct v3_shdw_pg_state * state = &(core->shdw_pg_state);
...
int vtlb_activate_shdw_pt(struct guest_info * core) {
...
int vtlb_invalidate_shdw_pt(struct guest_info * core) {
    return vtlb_activate_shdw_pt(core);
}


int vtlb_handle_pf(struct guest_info * core, addr_t fault_addr, pf_error_t error_code) {
...
int vtlb_handle_invlpg(struct guest_info * core, addr_t vaddr) {
...
struct v3_shdw_pg_impl vtlb_impl =  {
    .name = "VTLB",
    .init = vtlb_init,
    .deinit = vtlb_deinit,
